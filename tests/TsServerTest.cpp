...
int main() {
    try {
        ...
        std::cout << "🔌 [C++] Connecting to TypeScript server..." << std::endl;
        auto future = client.connect();
        
        // Wait for connection with timeout
        if (future.wait_for(std::chrono::seconds(10)) == std::future_status::ready) {
            std::cout << "✅ [C++] Connected to TypeScript server!" << std::endl;
            ...
            // Send test messages
            std::cout << "📤 [C++] Sending messages to TypeScript server..." << std::endl;
            client.send("/queue/cross-test", "Hello TypeScript from C++!");
            ...
            std::cout << "🎉 [C++] Cross-language test with TypeScript server completed successfully!" << std::endl;
        } else {
